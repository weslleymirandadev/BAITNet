...
static void usage_send(void)
{
    fprintf(stderr,
        ...
        "                  [--remote gw1,gw2:port]\n"
        "       ipv69 send <dst[:port]> [src_port] [payload]  (ifname = auto)\n"
        "\n"
        ...
        "When <ifname> is omitted (the first argument is an address), the\n"
        "default-route interface is used (\"auto\").\n"
        "\n"
        "Examples:\n"
...
static void usage_recv(void)
{
    fprintf(stderr,
        ...
        "       ipv69 recv [addr[:port]] [--remote gw:port]  (ifname = auto)\n"
        "\n"
        "Without an address, the lease is discovered silently via DHCP.\n"
        "With --remote, the address is derived from the identity (class C)\n"
        "and announced periodically so the gateway learns it. When <ifname>\n"
        "is omitted (the first argument is an address or a flag), the\n"
        "default-route interface is used (\"auto\").\n"
        "\n"
        "Examples:\n"
...
static void usage_icsp(void)
{
    fprintf(stderr,
        ...
        "        ipv69 icsp server :port             (ifname = auto)\n"
        "        ipv69 icsp client <dst:port> [msg]  (ifname = auto)\n"
        "\n"
