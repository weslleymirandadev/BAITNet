...
{
    uint8_t rec[BTLS_MAX_REC], msg[BTLS_MAX_PAYLOAD];
    size_t reclen, msglen;
    int rr;

    if (c->done) {
        /* application phase: decrypt what arrived */
        if (btls_recv_app(&c->btls, c->pend, c->pendlen, msg, &msglen) < 0)
            return -1;
        c->pendlen = 0;
        c->got_app = 1;
        printf("btls: app message received (%zu bytes): \"%.*s\"\n",
               msglen, (int)msglen, (char *)msg);
        if (is_server) {
            /* reply with one ack record — encrypted, like everything */
            static const char ack[] = "btls ack";
            if (btls_send_app(&c->btls, (const uint8_t *)ack,
                              sizeof(ack) - 1, rec, &reclen) < 0)
                return -1;
                      ...
                      uint8_t sk[64])
{
    ...
    while (time(NULL) < deadline) {
        ...
        if (c.done && !sent) {
            if (btls_send_app(&c.btls, (const uint8_t *)msg, strlen(msg),
                              rec, &reclen) < 0) {
